...
app.post('/signup', async (req, res) => {
  ...
  try {
    const hashedPW = await axios.get(`${AUTH_API_URL}/hashed-password/` + password);
...
app.post('/login', async (req, res) => {
  ...
  const response = await axios.get(
    `${AUTH_API_URL}/token/` + hashedPassword + '/' + password
  );
...
app.listen(8080);
console.log('users API listen on port ', 8080);