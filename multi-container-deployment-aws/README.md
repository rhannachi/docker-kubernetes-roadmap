...
## Ajouter un volume EFS

### 1. Création du groupe de sécurité pour EFS

- Crée un groupe de sécurité nommé `port-nfs-open-sg-0689271b`.
- Dans les règles entrantes, autorise le trafic entrant sur le port **2049** (NFS) uniquement provenant du **VPC par défaut** utilisés par tes instances ECS/services.
- Exemple de règle :
  - Type : NFS
  - Port : 2049
  - Source : SG du VPC par défaut (à adapter selon ta config).

![](./images/efs-1.png)

### 2. Création du système de fichiers EFS

- Va dans la console **EFS** et clique sur “Créer un système de fichiers”.
- Nom : `efs-storage`.
- Choisis le **VPC par défaut**.
- Coche toutes les zones de disponibilité utilisées par ton service ECS (recommandé pour la haute disponibilité).
- Pour chaque Point de Montage (Mount Target), associe le groupe de sécurité `port-nfs-open-sg-0689271b` précédemment créé.
- Termine la création.

![](./images/efs-2.png)

### 3. Mise à jour de la définition de tâche ECS

- Va dans la console **ECS** et sélectionne la définition de tâche : `goals-task`.
- Crée une **nouvelle révision** de cette définition.


### a) Ajout d’un volume EFS

- Cliquer sur “Ajouter un volume”.
- Nom du volume : par exemple `data` (utilisé pour le point de montage).
- Type de configuration : “Configurer lors de la création”.
- Type : EFS.
- Renseigne l’**ID du système de fichier** EFS (`efs-storage-...`).


### b) Ajout du point de montage dans le conteneur

- Dans la configuration du conteneur `mongodb`, va dans “Points de montage”.
- Ajoute un point de montage :
  - Volume : `data` (celui créé ci-dessus).
  - Chemin dans le conteneur : `/data/db`.


### 4. Validation

- Vérifie bien toutes les configurations, enregistre et valide la nouvelle révision de la définition de tâche.
- Redéploie ou met à jour ton service ECS afin qu’il utilise la dernière révision de la tâche.
